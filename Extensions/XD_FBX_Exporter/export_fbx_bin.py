...
from .fbx_utils import (
    ...
    # Animation.
    XD_AnimationCurveNodeWrapper,
    # Objects.
    XD_ObjectWrapper, fbx_name_class, ensure_object_not_in_edit_mode,
    # Top level.
...
def fbx_animations_do(scene_data, ref_id, f_start, f_end, start_zero, objects=None, force_keep=False):
    ...
    for ob_obj in objects:
        if ob_obj.parented_to_armature:
            continue
        ACNW = XD_AnimationCurveNodeWrapper
    ...
    for me, (me_key, _shapes_key, shapes) in scene_data.data_deformers_shape.items():
        ...
        for shape, (channel_key, geom_key, _shape_verts_co, _shape_verts_idx) in shapes.items():
            acnode = XD_AnimationCurveNodeWrapper(channel_key, 'SHAPE_KEY', force_key, force_sek, (0.0,))
    ...
    for cam_obj, cam_key in scene_data.data_cameras.items():
        ...
        acnode_lens = XD_AnimationCurveNodeWrapper(cam_key, 'CAMERA_FOCAL', force_key, force_sek, (cam.lens,))
        acnode_focus_distance = XD_AnimationCurveNodeWrapper(cam_key, 'CAMERA_FOCUS_DISTANCE', force_key,
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    for ob_obj in objects:
        ...
        if ob_obj.is_dupli:
            org_ob_obj = XD_ObjectWrapper(ob)  # We get the "real" object wrapper from that dupli instance.
...
def fbx_header_elements(root, scene_data, time=None):
    ...
    # Следующий код не актуален, т.к. вызывает ошибку в связи с переходом на manifest
    # from . import bl_info
