...
MESH_ATTRIBUTE_MATERIAL_INDEX = XD_AttributeDescription("material_index", 'INT', 'FACE')
MESH_ATTRIBUTE_POSITION = XD_AttributeDescription("position", 'FLOAT_VECTOR', 'POINT',
                                               is_required_check=lambda attributes: bool(attributes.id_data.vertices))
MESH_ATTRIBUTE_SHARP_EDGE = XD_AttributeDescription("sharp_edge", 'BOOLEAN', 'EDGE')
MESH_ATTRIBUTE_EDGE_VERTS = XD_AttributeDescription(".edge_verts", 'INT32_2D', 'EDGE',
                                                 is_required_check=lambda attributes: bool(attributes.id_data.edges))
MESH_ATTRIBUTE_CORNER_VERT = XD_AttributeDescription(".corner_vert", 'INT', 'CORNER',
                                                  is_required_check=lambda attributes: bool(attributes.id_data.loops))
MESH_ATTRIBUTE_CORNER_EDGE = XD_AttributeDescription(".corner_edge", 'INT', 'CORNER',
                                                  is_required_check=lambda attributes: bool(attributes.id_data.loops))
MESH_ATTRIBUTE_SHARP_FACE = XD_AttributeDescription("sharp_face", 'BOOLEAN', 'FACE')
...
def elem_empty(elem, name):
    sub_elem = encode_bin.XD_FBXElem(name)
    if elem is not None:
...
class XD_AnimationCurveNodeWrapper:
    """
...
class XD_MetaObjectWrapper(type):
...
class XD_ObjectWrapper(metaclass=XD_MetaObjectWrapper):
    """
    ...
    * Blender DepsgraphObjectInstance (for dulis).
    ...
    def get_parent(self):
        if self._tag == 'OB':
            if (self.bdata.parent and self.bdata.parent.type == 'ARMATURE' and
                ...
                if (bo_par):
                    return XD_ObjectWrapper(bo_par, self.bdata.parent)
                else:  # Fallback to mere object parenting.
                    return XD_ObjectWrapper(self.bdata.parent)
            else:
                # Mere object parenting.
                return XD_ObjectWrapper(self.bdata.parent)
        elif self._tag == 'DP':
            return XD_ObjectWrapper(self._ref)
        else:  # self._tag == 'BO'
            return XD_ObjectWrapper(self.bdata.parent, self._ref) or XD_ObjectWrapper(self._ref)
    ...
    def get_bones(self):
        if self._tag == 'OB' and self.bdata.type == 'ARMATURE':
            return (XD_ObjectWrapper(bo, self.bdata) for bo in self.bdata.data.bones)
        return ()
    ...
    def dupli_list_gen(self, depsgraph):
        if self._tag == 'OB' and self.bdata.is_instancer:
            return (XD_ObjectWrapper(dup) for dup in depsgraph.object_instances
                    if dup.parent and XD_ObjectWrapper(dup.parent.original) == self)
        return ()
