...
from .fbx_utils_threading import XD_MultiThreadedTaskConsumer
...
XD_FBXElem = namedtuple("XD_FBXElem", ("id", "props", "props_type", "elems"))
del namedtuple
...
def read_elem(read, tell, use_namedtuple, decompress_array_func, tell_file_offset=0):
    ...
    return XD_FBXElem(*args) if use_namedtuple else args
...
def parse(fn, use_namedtuple=True):
    ...
    multithread_decompress_array_cm = XD_MultiThreadedTaskConsumer.new_cpu_bound_cm(_decompress_and_insert_array)
    ...
    return XD_FBXElem(*args) if use_namedtuple else args, fbx_version
